$(document).ready(function () {
    ...
    const votingContractAddress = "0xE39d68C746Dc1ad5ed2EEbf9B8864d1BFd965631";
    ...
    async function login() {
        ...
        let backendPassword = CryptoJS
            .HmacSHA256(username, walletPassword).toString();
        try {
    ...
    async function register() {
        ...
        try {
            ...
            let jsonWallet = await wallet
                .encrypt(walletPassword, {}, showProgressBox);
            let backendPassword = CryptoJS
                .HmacSHA256(username, walletPassword).toString();
            ...
            showView('viewHome');
            showInfo(`User ${username} registered successfully. Please save your mnemonics: <b>${wallet.mnemonic.phrase}</b>`);
        } catch (err) {
    ...
    async function loadVotingResults() {
        try {
            ...
            for (let i = 0; i < candidatesCount; i++) {
                ...
                let li = $('<li>')
                    .html(`${candidate.candidate} -> ${candidate.votes} votes `);
