...
contract('Funding', async (accounts) => {
    ...
    it(
        'allows an owner to withdraw funds when goal is reached', 
        async () => {
            ...
            assert.isTrue(
                parseInt(finalBalance, 10) > parseInt(initBalance, 10)
            );
        }
    );

    it(
        'does not allow non-owners to withdraw funds', 
        async () => {
            const goal = web3.utils.toHex(100000000 * GWEI);
    ...
    it(
        'allows to withdraw funds after time is up and goal is not reached',
        async () => {
            await funding.donate({
                from: secondAccount, 
                value: 50000000 * GWEI
    ...
    it(
        'does not allow to withdraw funds after time is up and goal is reached',
        async () => {
            await funding.donate({ 
                from: secondAccount,
                value: 100000000 * GWEI
    ...
    it(
        'does not allow owner to withdraw funds before time is up and goal' + 
            'is not reached',
        async () => {
            await funding.donate({
                from: secondAccount,
